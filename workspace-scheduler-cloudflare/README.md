...
revocation, retention tombstones, and discovery receipts move only their task to
an exact due time through `/v1/kick`. The credential route is isolated from general retention work, so an
...
closed failure enums. The event-driven task and nullable receipt shape is the
current contract v2; other scheduler contracts are unsupported.
