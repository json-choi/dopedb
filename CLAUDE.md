...
않는다. 실제 Agent는 작업 범위와 완료 근거를 확인한다. 상세 경계는
...
- 45줄이 넘는 TS/TSX 화면·컴포넌트·lib 파일은 import 앞에 역할을 설명하는
  짧은 주석을 둔다. `pnpm check:role-comments`가 형식 누락만 막고 설명의
  정확성은 코드 리뷰가 판단한다. Rust는 줄 수와 무관한 `//!` 규칙이라 이
  검사 대상이 아니다. `pnpm build`가 self-test와 검사를 실행하며 CI도 같은
  build 경로를 사용한다.
