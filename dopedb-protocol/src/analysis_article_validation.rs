...
pub(crate) fn shared_create_is_valid(article: &SharedAnalysisArticleCreate) -> bool {
    contract_uuid(&article.id)
        ...
        && contract_uuid(&article.connection_id)
        && (1..=MAX_SAFE_INTEGER).contains(&article.connection_revision)
...
fn validate_definition(definition: &AnalysisArticleDefinition) -> bool {
