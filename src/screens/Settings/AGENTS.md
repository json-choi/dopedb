...
| `AgentTools/` | `index.tsx` — default-exported `AgentTools`: composes bounded Agent plugin (`AgentPluginSection`), Skill (`AgentSkillSection`), and install/self-test status via `useAgentToolsController` (all from `features/settings/agentTools/`). |
