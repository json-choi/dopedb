    ...
    match (policy_pool, engine) {
        (DbPool::Postgres(pool), Engine::Postgres) => {
            if access == ConnectionAccess::Schema {
                ...
                let owner: String = sqlx::query_scalar("SELECT CASE WHEN current_user = session_user THEN '' ELSE current_user::text END")
                    .fetch_one(&mut *conn).await?;
                super::super::gcp_schema_policy::prepare(&mut conn, &owner).await?;
                return Ok(());
            }
            let row = sqlx::query(
                "SELECT \
