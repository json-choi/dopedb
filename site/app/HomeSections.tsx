  ...
  return <>
    ...
    <section id="product" className="tw:relative tw:z-10 tw:scroll-mt-24 tw:bg-galaxy-reading tw:px-6 tw:pt-5 tw:pb-24 tw:md:px-12 tw:lg:pb-28">
      <div className="tw:mx-auto tw:max-w-[1264px]">
        ...
        <div className="tw:mt-7 tw:flex tw:flex-wrap tw:items-center tw:justify-between tw:gap-x-6 tw:gap-y-4 tw:border-b tw:border-hairline tw:pb-7 tw:text-[12px] tw:text-cream-muted">
          {[c.detailOne, c.detailTwo, c.detailThree].map(detail => (
            <span key={detail} className="tw:flex tw:items-center tw:gap-2.5">
              <span className="tw:size-1 tw:rounded-full tw:bg-signal/70" />{detail}
            </span>
          ))}
        </div>
      </div>
  ...
  return <>
    <section id="trust" className="tw:relative tw:z-10 tw:scroll-mt-24 tw:bg-night tw:px-6 tw:py-24 tw:md:px-12 tw:lg:py-32">
      <div className="tw:mx-auto tw:grid tw:max-w-[1264px] tw:gap-12 tw:lg:grid-cols-[minmax(0,.7fr)_minmax(0,1.3fr)] tw:lg:gap-20">
        <div><SectionLabel>{c.faqLabel}</SectionLabel><h2 className="tw:mt-7 tw:whitespace-pre-line tw:text-[clamp(36px,4vw,54px)] tw:leading-[1.17] tw:font-medium tw:tracking-[-0.05em]">{c.faqTitle}</h2><p className="tw:mt-6 tw:max-w-[360px] tw:text-[15px] tw:leading-[1.85] tw:text-cream-muted">{c.faqBody}</p><a href={`${repoUrl}/blob/main/docs/PRODUCT_POSITIONING.md`} target="_blank" rel="noreferrer" className="tw:mt-8 tw:inline-flex tw:min-h-11 tw:items-center tw:gap-3 tw:text-[12px] tw:text-signal tw:hover:underline">{c.docs} <Arrow diagonal /></a></div>
    ...
    <section id="download" className="tw:relative tw:z-10 tw:scroll-mt-24 tw:border-t tw:border-hairline tw:bg-galaxy-halo tw:px-6 tw:py-24 tw:md:px-12 tw:lg:py-32">
      <div className="tw:mx-auto tw:grid tw:max-w-[1264px] tw:items-center tw:gap-12 tw:lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] tw:lg:gap-20">
        ...
        <div>
          ...
          <details className="tw:mt-7 tw:border-y tw:border-hairline tw:py-4" open><summary className="tw:cursor-pointer tw:text-[12px] tw:text-cream">{c.install}</summary><div className="tw:mt-4 tw:space-y-3 tw:text-[12px] tw:leading-[1.85] tw:text-cream-muted"><p>{existing.download.sourceNote}</p><p>{existing.download.macSigning}</p><p><span className="tw:text-warning">{existing.download.windowsWarningTitle}</span><br />{existing.download.windowsWarningBody}</p></div></details>
