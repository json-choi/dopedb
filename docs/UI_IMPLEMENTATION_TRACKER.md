...
| Settings | `complete` | `features/settings`, `features/safetySettings` | Desktop `Settings → Safety` 하나에서 읽기·DML·DDL 단계를 선택하고, 관리자용 workspace 상한 + 기기 gate를 한 번의 적용 동작으로 fail-closed 저장하며 미적용 변경을 표시한다. 관리형 DDL은 exact `manage` grant와 검증된 provider schema lease가 있을 때만 열고, provider/연결/오류 화면은 상태와 이 화면 진입만 제공한다. 웹 DB 접근 화면은 같은 상한을 상태로만 표시하며 변경 control을 중복하지 않는다. compact viewport 검수 |
