...
| Workspace account authentication | `partial` | `features/workspaces/components/WorkspaceAccount`, native workspace loopback adapter, `workspace-cloud/app/auth/desktop` | Desktop은 HTTPS 웹의 계정 선택·승인 뒤 native의 임시 loopback listener와 PKCE code 교환으로 로그인한다. WebView는 공개 attempt handle과 terminal 결과만 받고 device polling을 실행하지 않는다. 취소·unmount·이전 시도의 늦은 완료를 현재 로그인과 분리하며, 성공 analytics와 권한 cache 갱신 경계를 유지한다. 비밀값 없는 앱 URL은 활성화 fallback만 소유하고 RFC 8628 endpoint는 호환용으로 유지한다. 현재 CLI에 독립 Workspace 로그인은 없다. 자동 계약·실제 packaged macOS·Windows의 Google 왕복, 계정 추가, 거부·취소·만료·재시작 검수와 운영 웹/앱 배포를 연결한 확인은 남아 있다. |
