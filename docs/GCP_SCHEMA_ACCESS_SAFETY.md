...
GCP PostgreSQL의 관리형 스키마 접근이 없는 연결은 관리자의 Safety 화면에서
`관리자 연결 필요` 상태와 `관리자 연결 설정` 동작을 표시한다. 이 동작은 기존
팀 연결을 변환하지 않고 주소·DB·TLS 모드만 채운 새 개인 로컬 연결 편집기를 연다.
사용자 이름·비밀번호·secret reference·provider target·추가 매개변수·Project grant와
쓰기 동의는 복사하지 않는다. 관리자는 자신의 자격 증명을 기기에 저장하고 기존
연결 테스트 및 Safety 쓰기/DDL 동의를 완료한다. 개인 연결을 Agent에 사용하려면
해당 Project 리소스로 명시적으로 선택해야 하며 팀 연결의 grant를 상속하지 않는다.

테이블 소유자가 CREATEROLE/CREATEDB나 Cloud SQL 관리자 역할을 가진 경우, 그 역할을
관리형 계정에 위임하여 검사를 통과시키지 않는다. 소유권과 기존 애플리케이션 권한은
유지하며, 관리자 개인 연결의 기존 승인 경계를 이용한다.
