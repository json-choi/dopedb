...
## Auth flows (app/auth/*)
- **Device-code flow** (`auth/device`, `auth/device/complete`): implements RFC 8628 for `dopedb agent start` / CLI pairing. `/auth/device` server-verifies the `user_code` via `auth.api.deviceVerify`, then shows sign-in, an account switcher (`DeviceAccountActions`), and approve/deny controls (`DeviceApproval`, calling `authClient.device.approve`/`.deny`). `/auth/device/complete` confirms the outcome and its `DeviceCompletionAction` client component immediately redirects to `desktopWorkspaceLoginCallbackUrl` — verified in code, this is the exact handoff back to the local Desktop app / CLI that repo-root docs describe.
